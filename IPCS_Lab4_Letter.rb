...
def countCharAmounts(file_contents)
    char_amount = Hash.new(0)
    file_contents.each_char { |char| char_amount[char] += 1 }
    char_amount.sort.to_h
end
...
def main()
    begin
        ...
        file_names.each do |file_name|
            ...
            file_contents = File.read(file_path).upcase.gsub(/[A-z,∆[:digit:][:punct:]\n]/, '')
            char_amounts = countCharAmounts(file_contents)
            ...
            char_amounts.each { 
                |char, amount|
                puts "#{iterator+=1})\t|\t#{convertSpecialCharacters(char)}\t|\t#{amount}\t|\t#{(amount.to_f/char_amounts.values.sum).round(5)}\t|\t#{(amount.to_f/char_amounts.values.sum * 100).round(5)}\t|" }

            amount_of_information = (entropyCount(char_amounts)*char_amounts.values.sum)/8
            puts "\nЗагальна кількість символів: #{char_amounts.values.sum}"
            ...
            CSV.open("CSV/" + file_name.sub(/\.txt$/, '') + ".csv",'w') do |csv|
                csv << ["№","Символ","Кільк.","Імовірність","%"]
                iterator = 0
                char_amounts.each do |row|
                    csv << [iterator+=1] + [convertSpecialCharacters(row[0])] + [row[1]] + [(row[1].to_f/char_amounts.values.sum).round(5)] + [(row[1].to_f/char_amounts.values.sum * 100).round(5)]
                end
            end
